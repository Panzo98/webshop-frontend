...
export default function SlideCart({ isCartOpen, setIsCartOpen }) {
  const cartItems = useSelector((state) => state.cartReducer.cart);
  const dispatch = useDispatch();
  const [rightPosition, setRightPosition] = useState(-416);
  ...
  return (
    <div
      ...
      className="fixed top-0 transition-all duration-200 w-[26rem] bg-white flex flex-col h-auto"
    >
      <div className="p-8">
        <div className="flex justify-between items-center">
          <span className="text-black tracking-widest font-poppins text-2xl font-semibold leading-normal w-10/12">
            Shopping Cart
          </span>
        ...
        <div className="w-full h-[1px] mt-8 mb-11">
          <div className="w-9/12 h-full bg-gray-200"></div>
        </div>
            ...
            return (
              <div className="w-full mb-5 flex">
                <div className="w-10/12 flex">
                  <img
                    className="h-[100px] w-[100px] rounded mr-8"
                    src={item.image}
                    alt="image"
                  />
                  <div className="h-full flex flex-col justify-center">
                    <div className="mb-2 pr-3 text-black font-poppins text-base font-normal leading-normal">
                      {item.title}
                    </div>
                    <div className="flex items-center">
                      <span className="text-black font-poppins text-base font-light leading-normal">
                        {item.quantity}
                      </span>
                      <span className="text-black font-poppins text-xs font-light leading-normal mx-4">
                        X
                      </span>
                      <span className="text-primary-color font-poppins text-sm font-medium leading-normal">
                        $ {item.price}
                      </span>
                    </div>
                  </div>
        ...
        <div className="flex justify-between pr-10">
          <span className="text-black font-poppins text-base font-normal leading-normal tracking-widest">
            Subtotal
          </span>
          <span className="text-primary-color font-poppins font-medium leading-normal">
            $ {subtotal}
          </span>
        </div>
      </div>
      <div className="border-t-[1px] p-7">
        <button className="rounded-full border border-black px-8 py-[6px] mr-4 text-black font-poppins text-xs font-normal leading-normal">
          Cart
        </button>
        <button className="rounded-full border border-black px-8 py-[6px] text-black font-poppins text-xs font-normal leading-normal">
          Checkout
        </button>
      </div>
    </div>
